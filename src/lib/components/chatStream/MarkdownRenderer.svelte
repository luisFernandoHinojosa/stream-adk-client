...
<script lang="ts">
	...
	$: {
		// Reactiva cuando markdownText cambia
		if (markdownText) {
			marked.setOptions({
				breaks: true,
				gfm: true
			...
			// Aplicar estilos específicos después del renderizado
			setTimeout(() => {
				const container = document.querySelector('.markdown-content');
				if (container) {
					const listItems = container.querySelectorAll('li');
					listItems.forEach((li) => {
						...
						li.classList.remove('total-price', 'price-item');
...
<div>
	<div class="markdown-content prose prose-base max-w-none md:prose-lg">
		{@html htmlContent}
	</div>
</div>
